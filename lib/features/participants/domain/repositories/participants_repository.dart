...
/// Interface de repositório para a entidade Participant.
///
...
/// - Certifique-se de que a entidade Participant possui métodos de conversão robustos (ex: aceitar id como int ou string, datas como DateTime ou String).
...
/// - Consulte os arquivos de debug do projeto (ex: participants_cache_debug_prompt.md, supabase_init_debug_prompt.md, supabase_rls_remediation.md) para exemplos de logs, prints e soluções de problemas reais.
abstract class ParticipantsRepository {
  ...
  Future<List<Participant>> loadFromCache();
  ...
  /// Este método retorna todos os participantes disponíveis no cache.
  /// Boas práticas:
  ...
  /// Este método retorna apenas os participantes destacados/em destaque.
  /// Boas práticas:
  /// - Assegure-se de que a entidade Participant possui um campo para marcar participantes em destaque.
  /// - Use este método para exibir seções especiais na UI (ex: "Participantes em Destaque").
  /// - O filtro deve ocorrer localmente no cache, não no servidor.
  Future<List<Participant>> listFeatured();

  /// Opcional: busca direta por ID no cache.
  /// Este método busca um participante específico pelo seu ID.
  /// Boas práticas:
  /// - Verifique se o cache contém o ID antes de fazer requisições remotas.
  /// - Retorne null se o participante não for encontrado no cache.
  /// - Para buscas que necessitam de dados remotos, considere implementar uma versão remota desta interface.
  Future<Participant?> getById(int id);
}

/*
// Exemplo de uso:
final repo = MinhaImplementacaoDeParticipantsRepository();
...
// Referências úteis:
// - participants_cache_debug_prompt.md
