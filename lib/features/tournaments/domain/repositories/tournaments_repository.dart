...
/// Interface de repositório para a entidade Tournament.
///
...
/// - Certifique-se de que a entidade Tournament possui métodos de conversão robustos (ex: aceitar id como int ou string, datas como DateTime ou String).
...
/// - Consulte os arquivos de debug do projeto (ex: tournaments_cache_debug_prompt.md, supabase_init_debug_prompt.md, supabase_rls_remediation.md) para exemplos de logs, prints e soluções de problemas reais.
abstract class TournamentsRepository {
  ...
  Future<List<Tournament>> loadFromCache();
  ...
  /// Este método retorna todos os torneios disponíveis no cache.
  /// Boas práticas:
  ...
  /// Este método retorna apenas os torneios destacados/em destaque.
  /// Boas práticas:
  /// - Assegure-se de que a entidade Tournament possui um campo para marcar torneios em destaque.
  /// - Use este método para exibir seções especiais na UI (ex: "Torneios em Destaque").
  /// - O filtro deve ocorrer localmente no cache, não no servidor.
  Future<List<Tournament>> listFeatured();

  /// Opcional: busca direta por ID no cache.
  /// Este método busca um torneio específico pelo seu ID.
  /// Boas práticas:
  /// - Verifique se o cache contém o ID antes de fazer requisições remotas.
  /// - Retorne null se o torneio não for encontrado no cache.
...
/*
// Exemplo de uso:
final repo = MinhaImplementacaoDeTournamentsRepository();
...
// Referências úteis:
// - tournaments_cache_debug_prompt.md
