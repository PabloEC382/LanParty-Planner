...
/// - Certifique-se de que a entidade Game possui métodos de conversão robustos (ex: aceitar id como int ou string, datas como DateTime ou String).
...
/// - Consulte os arquivos de debug do projeto (ex: games_cache_debug_prompt.md, supabase_init_debug_prompt.md, supabase_rls_remediation.md) para exemplos de logs, prints e soluções de problemas reais.
abstract class GamesRepository {
  ...
  Future<List<Game>> loadFromCache();
  ...
  /// Este método retorna todos os games disponíveis no cache.
  /// Boas práticas:
  ...
  /// Este método retorna apenas os games destacados/em destaque.
  /// Boas práticas:
  /// - Assegure-se de que a entidade Game possui um campo para marcar games em destaque.
  /// - Use este método para exibir seções especiais na UI (ex: "Jogos em Destaque").
  /// - O filtro deve ocorrer localmente no cache, não no servidor.
  Future<List<Game>> listFeatured();

  /// Opcional: busca direta por ID no cache.
  /// Este método busca um game específico pelo seu ID.
  /// Boas práticas:
  /// - Verifique se o cache contém o ID antes de fazer requisições remotas.
  /// - Retorne null se o game não for encontrado no cache.
...
/*
// Exemplo de uso:
final repo = MinhaImplementacaoDeGamesRepository();
